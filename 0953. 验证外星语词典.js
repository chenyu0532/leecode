...
var small = function(pre, current, map){
	let len = Math.min(pre.length, current.length);
	for(let j = 0; j < len; j++) {
		if(map.get(pre[j]) != map.get(current[j])) {
			return map.get(pre[j]) < map.get(current[j]);
		}
	}
	return pre.length <= current.length;
};
