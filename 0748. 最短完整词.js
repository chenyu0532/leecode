...
var shortestCompletingWord = function(licensePlate, words) {
    ...
    for(let i = 0; i < licensePlate.length; i++) {
    	let temp = licensePlate[i].toLowerCase();
    	if ((temp >= 'a' && temp <= 'z')) {
    		map1.has(temp)? map1.set(temp, map1.get(temp) + 1): map1.set(temp, 1);
    	}
    }
    
    //////用了如下的排序，发现sort并不会完全只是按照长度来排序，除了长度应该还有别的因素，所以不用这个排序了
    ...
    for(let i = 0; i < words.length; i++) {
    	...
    	for(let j = 0; j < words[i].length; j++) {
    		let temp = words[i][j];
			map2.has(temp)? map2.set(temp, map2.get(temp) + 1): map2.set(temp, 1);
    	}
    	//console.log(map2);
